...
def _create_user_db(
    db_session, username="user", password="Test1234", role=UserRole.USER, active=True
):
...
class TestPasswordChange:
    ...
    def test_change_creates_audit_log(self, client, db_session):
        ...
        logs = db_session.query(AuditLog).filter(AuditLog.action == "PASSWORD_CHANGED").all()
        assert len(logs) >= 1
...
class TestPasswordReset:
    ...
    def test_reset_creates_audit_log(self, client, db_session):
        ...
        logs = db_session.query(AuditLog).filter(AuditLog.action == "PASSWORD_RESET_BY_ADMIN").all()
        assert len(logs) >= 1
